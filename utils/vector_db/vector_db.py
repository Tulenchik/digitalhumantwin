...
class VectorDB:
    ...
    def load(self):
        if os.path.exists(self.db_file):
            ...
            print(f"Vector DB file {self.db_file} does not exist. Starting with empty database.")
            self.entries = []
    ...
    def get_context_string(self, query_embedding: list, top_n: int = 4) -> str:
        # If the query_embedding is empty or None, return empty string
...
# Create a dummy vector database that won't do any actual embedding work
# but will satisfy the interface requirements
class DummyVectorDB(VectorDB):
    def __init__(self):
        self.entries = []
        print("DummyVectorDB initialized (for debugging)")
    ...
    def add_entry(self, embedding, text, metadata=None):
        pass
        
    def search(self, query_embedding, top_n=4):
        return []
        
    def get_context_string(self, query_embedding, top_n=4):
        return ""

# Create an instance of the dummy vector DB to prevent errors
# vector_db = VectorDB()
